...
## [Issue #7] Drop Zone Transparent Overlay Window (Visual Shell)
...
### Acceptance criteria
- [ ] Secondary transparent, frameless window is spawned on application startup.
- [ ] Click-through is active (users can click, scroll, and drag windows *behind* the Drop Zone visual frame).
- [ ] Renders a visually distinct overlay (dashed border / semi-transparent area) indicating the Drop Zone region.
- [ ] Repositions and rescales itself correctly upon display layout/scaling changes.
...
## [Issue #8] Win32 Drag/Drop Detection & Rectangle Intersection
...
### Acceptance criteria
- [ ] Win32 service exposes `GetForegroundWindow` and mouse state checks.
- [ ] Log or trigger an event in the main process when the foreground window's rectangle overlaps the Drop Zone region upon drag release.
- [ ] Does not trigger detection for SpaceToggle's own windows.
...
### Acceptance criteria
- [ ] Dragging and dropping an eligible window into the Drop Zone applies 50% opacity.
- [ ] Dragging and dropping Chrome/Edge falls back to off-screen movement if configured, hiding the window.
- [ ] Captured windows are tracked in the state and can be restored to their original visual styling/position.
...
### Acceptance criteria
- [ ] Application state persists window metadata instantly.
- [ ] Right-clicking the system tray icon and selecting "Force Restore All" restores all modified windows.
- [ ] Killing the application via Task Manager leaves windows in their modified states, but launching SpaceToggle again automatically restores all modified windows to normal.
